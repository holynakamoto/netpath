...
def fetch_coverage_by_country(api_key: str) -> dict[str, dict]:
    ...
    def _accumulate(start_url: str, key: str, country_field: str) -> None:
        ...
        while url:
            try:
                ...
                msg = f"Atlas API request failed while fetching {key}: {exc}"
                if isinstance(exc, requests.HTTPError) and exc.response is not None \
                        and exc.response.status_code in (401, 403):
                    msg += "\n    Your API key may be invalid — check it at https://atlas.ripe.net/keys/"
                console.print(f"  [yellow]⚠[/yellow] {msg}")
                break
            for item in data.get("results", []):
                cc = item.get(country_field, "")
                if cc:
    ...
    with Progress(SpinnerColumn(), TextColumn("{task.description}"),
        ...
        _accumulate(
            f"{_ANCHORS_BASE}/probes/?status=1&page_size=500",
            "probes",
            "country_code",
        )
    ...
    with Progress(SpinnerColumn(), TextColumn("{task.description}"),
        ...
        # Anchor objects report their location as "country", not "country_code".
        _accumulate(
            f"{_ANCHORS_BASE}/anchors/?status=1&page_size=200",
            "anchors",
            "country",
        )
